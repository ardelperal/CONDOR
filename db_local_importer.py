...
class DBLocalImporter:
    ...
    def __init__(self, custom_local_dir: str = None):
        """
        Inicializar configuración
        
        Args:
            custom_local_dir: Directorio personalizado para bases de datos locales.
                            Si no se especifica, usa LOCAL_DB_DIR del .env o 'dbs-locales'
        """
        ...
        # Usar directorio personalizado si se proporciona, sino usar .env o default
        if custom_local_dir:
            self.local_db_dir = custom_local_dir
            self.logger.info(f"[CONFIG] Usando directorio personalizado: {custom_local_dir}")
        else:
            self.local_db_dir = os.getenv('LOCAL_DB_DIR', 'dbs-locales')
            self.logger.debug(f"[CONFIG] Usando directorio por defecto: {self.local_db_dir}")
...
Ejemplos de uso:
  ...
  python db_local_importer.py --local-dir "C:\\MisBDs"     # Usar directorio personalizado
  python db_local_importer.py --local-dir "D:\\Datos" --links-only  # Directorio personalizado + solo vínculos
        """
    )
    ...
    parser.add_argument(
        '--local-dir',
        type=str,
        help='Directorio personalizado para guardar las bases de datos locales. Si no se especifica, usa LOCAL_DB_DIR del .env o "dbs-locales"'
    )
    ...
    try:
        # Crear instancia con directorio personalizado si se especifica
        importer = DBLocalImporter(custom_local_dir=args.local_dir)
