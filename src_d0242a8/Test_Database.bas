...
Private Sub SetupValidDatosPCData()
    With m_TestDatosPC
        .ID = 0 ' Nuevo registro
        .SolicitudID = 0 ' Se asignar├í despu├®s
        .NumeroExpediente = "EXP-2025-001"
        .TipoSolicitud = "PC"
        .DescripcionCambio = "Cambio en el proceso de validaci├│n"
...
Public Function Test_Database_RunAll() As String
    ...
    ' Ejecutar todas las pruebas b├ísicas
...
Public Function Test_SaveSolicitudPC_TransactionFail_Rollback() As Boolean
    ...
    ' Act & Assert
    ' Verificamos que el mock simule fallo de transacci├│n
...
Public Function Test_SaveSolicitudPC_InvalidData_HandlesError() As Boolean
    ...
    ' Arrange
    ConfigureMockToFail 3061, "Datos inv├ílidos"
    SetupValidSolicitudData
    SetupValidDatosPCData
    ' Datos inv├ílidos
...
Public Function Test_Integration_ErrorHandling() As Boolean
    ...
    ' Arrange
    ConfigureMockToFail 3197, "Error en la transacci├│n"
    SetupValidSolicitudData
    SetupValidDatosPCData
...
Public Function Test_EdgeCase_SpecialCharacters() As Boolean
    ...
    m_TestSolicitud.Observaciones = "Prueba con 'comillas' y ""comillas dobles"" y s├¡mbolos: @#$%^&*()"
    m_TestDatosPC.DescripcionCambio = "Descripci├│n con ├▒, ├í, ├®, ├¡, ├│, ├║ y ┬┐┬ícaracteres especiales!?"
    
    ' Act & Assert
    Test_EdgeCase_SpecialCharacters = (InStr(m_TestSolicitud.Observaciones, "'") > 0) And _
                                    (InStr(m_TestDatosPC.DescripcionCambio, "├▒") > 0)
...
Public Function RunDatabaseTests() As String
    ...
    If Test_GetSolicitudData_ValidID_ReturnsRecordset() Then
        ...
        resultado = resultado & "Ô£ô Test_GetSolicitudData_ValidID_ReturnsRecordset" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_GetSolicitudData_ValidID_ReturnsRecordset" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_GetSolicitudData_InvalidID_ReturnsNothing() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_GetSolicitudData_InvalidID_ReturnsNothing" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_GetSolicitudData_InvalidID_ReturnsNothing" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_GetSolicitudData_DatabaseError_HandlesGracefully() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_GetSolicitudData_DatabaseError_HandlesGracefully" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_GetSolicitudData_DatabaseError_HandlesGracefully" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SaveSolicitudPC_NewRecord_Success() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SaveSolicitudPC_NewRecord_Success" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SaveSolicitudPC_NewRecord_Success" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SaveSolicitudPC_UpdateRecord_Success() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SaveSolicitudPC_UpdateRecord_Success" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SaveSolicitudPC_UpdateRecord_Success" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SaveSolicitudPC_TransactionFail_Rollback() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SaveSolicitudPC_TransactionFail_Rollback" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SaveSolicitudPC_TransactionFail_Rollback" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SaveSolicitudPC_InvalidData_HandlesError() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SaveSolicitudPC_InvalidData_HandlesError" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SaveSolicitudPC_InvalidData_HandlesError" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SolicitudExists_ValidID_ReturnsTrue() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SolicitudExists_ValidID_ReturnsTrue" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SolicitudExists_ValidID_ReturnsTrue" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SolicitudExists_InvalidID_ReturnsFalse() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SolicitudExists_InvalidID_ReturnsFalse" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SolicitudExists_InvalidID_ReturnsFalse" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SolicitudExists_ZeroID_ReturnsFalse() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SolicitudExists_ZeroID_ReturnsFalse" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SolicitudExists_ZeroID_ReturnsFalse" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_SolicitudExists_DatabaseError_ReturnsFalse() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_SolicitudExists_DatabaseError_ReturnsFalse" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_SolicitudExists_DatabaseError_ReturnsFalse" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_Integration_SaveAndRetrieve() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_Integration_SaveAndRetrieve" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_Integration_SaveAndRetrieve" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_Integration_SaveUpdateSave() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_Integration_SaveUpdateSave" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_Integration_SaveUpdateSave" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_Integration_ErrorHandling() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_Integration_ErrorHandling" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_Integration_ErrorHandling" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_EdgeCase_LargeDataset() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_EdgeCase_LargeDataset" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_EdgeCase_LargeDataset" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_EdgeCase_VeryLongStrings() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_EdgeCase_VeryLongStrings" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_EdgeCase_VeryLongStrings" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_EdgeCase_SpecialCharacters() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_EdgeCase_SpecialCharacters" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_EdgeCase_SpecialCharacters" & vbCrLf
    End If
    
    totalTests = totalTests + 1
    If Test_EdgeCase_ConcurrentOperations() Then
        passedTests = passedTests + 1
        resultado = resultado & "Ô£ô Test_EdgeCase_ConcurrentOperations" & vbCrLf
    Else
        resultado = resultado & "Ô£ù Test_EdgeCase_ConcurrentOperations" & vbCrLf
    End If
