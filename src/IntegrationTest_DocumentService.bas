...
Private Sub InitializeRealDependencies()
    ...
    ' 2. Repositorios
    Dim tempMapeoRepo As New CMapeoRepository
    tempMapeoRepo.Initialize m_Config, m_ErrorHandler
    Set m_MapeoRepo = tempMapeoRepo

    Dim tempSolicitudRepo As New CSolicitudRepository
    tempSolicitudRepo.Initialize m_Config
    Set m_SolicitudRepo = tempSolicitudRepo

    Dim tempExpedienteRepo As New CExpedienteRepository
    tempExpedienteRepo.Initialize m_Config, m_ErrorHandler
    Set m_ExpedienteRepo = tempExpedienteRepo

    ' 3. Servicios de Infraestructura
    Set m_OperationLogger = modOperationLoggerFactory.CreateOperationLogger(m_ErrorHandler)

    Dim wordApp As Object
    Set wordApp = CreateObject("Word.Application")
    Dim tempWordManager As New CWordManager
    tempWordManager.Initialize wordApp, m_ErrorHandler
    Set m_WordManager = tempWordManager

    ' 4. Servicio Principal a Probar
    Dim tempDocService As New CDocumentService
    tempDocService.Initialize m_Config, m_SolicitudRepo, m_OperationLogger, m_WordManager, m_MapeoRepo, m_ErrorHandler
    Set m_DocumentService = tempDocService
End Sub
