...
' =====================================================
Public Sub App_Start(ByVal errorHandler As IErrorHandlerService, Optional ByVal testUserEmail As String = "")
    On Error GoTo ErrorHandler

    ' 1. Obtener el email del usuario actual
    Dim UserEmail As String
    If testUserEmail <> "" Then
        ' Usar email de prueba si se proporciona (modo test)
        UserEmail = testUserEmail
    Else
        ' Usar lógica normal de producción/desarrollo
        UserEmail = GetCurrentUserEmail()
    End If

    If UserEmail = "" Then
        errorHandler.LogError vbObjectError + 514, "No se pudo obtener el email del usuario para la autenticación.", "modAppManager.App_Start", True ' CRITICAL_STARTUP_FAILURE
        Exit Sub
    End If
    ...
    Select Case g_CurrentUserRole
        ...
        Case Rol_Desconocido
            errorHandler.LogError vbObjectError + 515, "El usuario '" & UserEmail & "' no tiene un rol definido en el sistema.", "modAppManager.App_Start", True ' CRITICAL_AUTH_FAILURE
    End Select
    
    Exit Sub
    
ErrorHandler:
    errorHandler.LogError Err.Number, Err.Description, "modAppManager.App_Start", True ' CRITICAL_STARTUP_ERROR
End Sub
